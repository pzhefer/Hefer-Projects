...
import { Plus, Edit2, Trash2, X, Save, Building2, Box, Tag, FolderTree, ChevronRight, ChevronDown } from 'lucide-react';
...
type SettingSection = 'stores' | 'bins' | 'categories' | 'item_types';
    ...
    try {
      ...
      const [storesResult, binsResult, categoriesResult] = await Promise.all([
        supabase.from('stock_stores').select(`
          *,
          city_name:global_cities!city_id(name),
          state_name:global_states!state_id(name),
          country_name:global_countries!country_id(name)
        `).order('code'),
        supabase.from('stock_bins').select('*').order('code'),
      ...
      if (binsResult.data) setBins(binsResult.data);
    ...
    try {
      if (activeTab === 'stores') {
        const storeData = {
          ...
          city: formData.get('city_id') as string || null,
          state: formData.get('state_id') as string || null,
          postal_code: formData.get('postal_code') as string || null,
          country: formData.get('country_id') as string || null,
